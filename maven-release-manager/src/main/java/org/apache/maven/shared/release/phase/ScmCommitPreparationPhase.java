...
public class ScmCommitPreparationPhase
    ...
    protected void runLogic(ReleaseDescriptor releaseDescriptor, ReleaseEnvironment releaseEnvironment, List reactorProjects, ReleaseResult result, boolean simulating)
            throws ReleaseScmCommandException, ReleaseExecutionException, ReleaseScmRepositoryException
    {
        ...
        if (releaseDescriptor.isSuppressCommitBeforeTagOrBranch())
        {
            ...
            {
                logInfo( result, "Modified POMs are not committed because suppressCommitBeforeTagOrBranch is set to false." );
            }
        }
